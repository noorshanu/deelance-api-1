...
const deleteLoginActivityRecord = catchAsync(async (req, res) => {
    const { recordId  } = await pick(req.params, ['recordId'])
    const userId= req.user?.id;
   
    const insertResult = await followerServices.deleteLoginActivityRecord({
        userId, recordId
    });
