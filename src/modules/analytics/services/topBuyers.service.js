 ...
 */
const topBuyer = async (startDateParam, endDateParam, page, limit, filter, sort) => {
    try {
        ...
        const uniqueCreatorCount = await Sales.aggregate([
            ...
            },
            {
                $group: {
                    ...
                    buyerCount: { $sum: 1 } // Count the number of students for each subject
                }
            },
            {
                $sort: { buyerCount: -1 } // Sort by studentCount in descending order
            },
            {
                $project: {
                    "userObj.firstName": 1,
