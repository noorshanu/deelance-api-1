 ...
 */
const getTopCreator = async (fromDate, toDate,page,limit,filter,sort) => {
    try {
        ...
        const getTopCreator = await TransactionModel.aggregate(aggregateQuery).skip(skip).limit(length)
        const totalResults = await TransactionModel.countDocuments(filterQuery);
        const totalPages = Math.ceil(totalResults / length);
        if (getTopCreator.length > 0) {
            return { data: getTopCreator, totalResults, totalPages, page:start, limit:length,status: true, code: 200 };
        }
        else {
            return { data: "No Top Creator found", status: false, code: 400 };
        }
